...
public class AtomicDbTicketIssueStrategy implements TicketIssueStrategy {
    ...
    public TicketReadyResponse issue(TicketRequest request, String orderId) {
        TicketType ticketType = OptionalUtil.getOrElseThrow(
                ticketTypeRepository.findById(request.getTicketTypeId()), "존재하지 않는 티켓입니다.");
        Member member = OptionalUtil.getOrElseThrow(
                memberRepository.findById(request.getMemberId()), "존재하지 않는 회원입니다.");

        long updatedRows = ticketTypeRepository.increaseTicketAtomically(request.getTicketTypeId());
        if (updatedRows == 0) {
